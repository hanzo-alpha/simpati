...
<template>
    <component :is="isAdmin ? AdminLayout : AppLayout" title="Peringkat Kehadiran"
        subtitle="Leaderboard disiplin & performa presensi ASN Kabupaten Soppeng">
        <template #actions>
            <div class="flex items-center gap-1 rounded-none border border-border bg-card p-1 shadow-xs">
                <button v-for="tab in periodTabs" :key="tab.value" @click="changePeriod(tab.value)"
                    class="cursor-pointer rounded-none px-3.5 py-1.5 text-xs font-semibold transition-all" :class="activePeriod === tab.value
        ...
        <div class="space-y-6 pb-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard icon="alert" label="Peringkat Presensi Anda" :value="`#${myRank.rank}`"
                    :sub="`dari total ${myRank.totalAsn} ASN`" color="teal" />
                <StatCard icon="check" label="Skor Performa Anda" :value="`${myRank.score} Pts`"
                    sub="Skor komposit presensi" color="green" />
                <StatCard icon="file" label="Lencana Disiplin" :value="myRank.badge" sub="Status evaluasi bulanan"
                    color="yellow" />
                <StatCard icon="users" label="Total ASN Evaluasi" :value="`${myRank.totalAsn} ASN`" sub="Pemkab Soppeng"
                    color="blue" />
            </div>
                    ...
                    <Card v-if="topThree[1]"
                        ...
                        <CardContent class="space-y-3 p-5 pt-6">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="font-mono text-xl font-bold text-slate-300">{{ topThree[1].score }}
                                    Pts</span>
                            </div>
                    ...
                    <Card v-if="topThree[2]"
                        ...
                        <CardContent class="space-y-3 p-5 pt-6">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="font-mono text-xl font-bold text-amber-600">{{ topThree[2].score }}
                                    Pts</span>
                            </div>
            ...
            <Card class="rounded-none border border-border bg-card text-card-foreground shadow-xs">
                <CardHeader class="border-b border-border/60 p-5">
                    <CardTitle
                        ...
                        <span>Daftar Leaderboard Peringkat Kehadiran ASN (Top
                            50)</span>
                    </CardTitle>
                </CardHeader>
                        ...
                        <table class="w-full text-xs">
                            <thead>
                                <tr class="border-b border-border bg-muted/30 text-left text-muted-foreground">
                                    <th class="w-20 px-5 py-3.5 text-center text-xs font-bold tracking-wider uppercase">
                                        Peringkat
                                    </th>
                                    <th class="px-5 py-3.5 text-xs font-bold tracking-wider uppercase">
                                        Nama Pegawai ASN
                                    </th>
                                    <th class="px-5 py-3.5 text-xs font-bold tracking-wider uppercase">
                                        OPD Utama
                                    </th>
                                    <th class="px-5 py-3.5 text-center text-xs font-bold tracking-wider uppercase">
                                        Hadir Tepat Waktu
                                    </th>
                                    <th class="px-5 py-3.5 text-center text-xs font-bold tracking-wider uppercase">
                                        Terlambat
                            ...
                            <tbody class="divide-y divide-border/40">
                                <tr v-for="(person, index) in rankings" :key="person.id"
                                    class="transition-colors hover:bg-muted/40" :class="{
                                        'bg-emerald-500/10 font-bold dark:bg-emerald-500/15':
                                            person.isMe,
                                    }">
                                    <td class="px-5 py-3.5 text-center">
                                        <div class="mx-auto flex h-7 w-8 items-center justify-center rounded-none border font-mono text-xs font-bold"
                                            :class="index === 0
                                                ...
                                                ">
                                            <Trophy v-if="index === 0" class="h-3.5 w-3.5" />
                                            <span v-else>#{{ index + 1 }}</span>
                                        </div>
                                    </td>
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center gap-2">
                                            <span class="text-xs font-bold text-foreground">{{ person.name }}</span>
                                            <Badge v-if="person.isMe" variant="default"
                                                class="rounded-none bg-emerald-600 px-1.5 py-0 text-[9px]">Anda</Badge>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-5 py-3.5 text-center">
                                        <Badge variant="outline"
                                            class="rounded-none px-2 py-0.5 text-[10px] font-semibold" :class="person.badge.includes(
                                                'Sangat Baik',
                                            ) ||
                                    ...
                                    <td class="px-5 py-3.5 text-center font-mono text-sm font-bold text-primary">
                                        {{ person.score }} Pts
                                    </td>
                                </tr>
                            </tbody>
